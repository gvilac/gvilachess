...
public class RookMovement implements Movement {
    ...
    public List<Move> getAllMovements(Position position, BoardData boardData, Color color) {
        ...
        possiblePositions.putAll(MovementUtils.getOneDirectionMovements(position, boardData, color, new HashMap<>(), 0, 1, BoardData.BOARD_SIZE));
        possiblePositions.putAll(MovementUtils.getOneDirectionMovements(position, boardData, color, new HashMap<>(), 0, -1, BoardData.BOARD_SIZE));
        possiblePositions.putAll(MovementUtils.getOneDirectionMovements(position, boardData, color, new HashMap<>(), 1, 0, BoardData.BOARD_SIZE));
        possiblePositions.putAll(MovementUtils.getOneDirectionMovements(position, boardData, color, new HashMap<>(), -1, 0, BoardData.BOARD_SIZE));
